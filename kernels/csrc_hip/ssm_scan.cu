...
/*
 * Mamba-3 SSM selective scan -- HIP kernels (ported from CUDA, target gfx1201/RDNA4).
 *
...
__global__ void ssm_scan_bwd_kernel(
        ...
        for (int t = t_end - 1; t >= t_start; t--) {
            ...
            // DD-RoPE: compute per-position d_theta via reverse-cumsum of d_angle
            {
                ...
                if (tid < half_d_state) {
                    ...
                    for (int offset = 16; offset > 0; offset >>= 1)
                        d_dt_from_rope += __shfl_xor_sync(FULL_WARP_MASK, d_dt_from_rope, offset);
                    if (tid == 0) {
            ...
            // d_dt_pos/d_dt_raw = sigmoid(dt_raw + dt_bias)
            {
                ...
                for (int offset = 16; offset > 0; offset >>= 1) {
                    d_A_val_local += __shfl_xor_sync(FULL_WARP_MASK, d_A_val_local, offset);
                    d_dt_local += __shfl_xor_sync(FULL_WARP_MASK, d_dt_local, offset);
                }
                if (lane_id == 0) {
    ...
    unsigned long long warp_reduce_mask = (1ull << n_warps) - 1;  // only n_warps threads participate
...
void ssm_scan_bwd_gpu_v2(
    ...
    hipMemset(d_ddt, 0, dt_size);
    hipMemset(d_db, 0, bc_size);
    hipMemset(d_dc, 0, bc_size);
    hipMemset(d_d_lambda, 0, dt_size);
    hipMemset(d_d_A_vals, 0, dt_size);
