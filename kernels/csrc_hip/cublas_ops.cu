...
/*
 * hipBLAS matmul wrapper for native training (ported from cuBLAS).
 *
 * All linear layers (in_proj, out_proj, output projection) use hipBLAS sgemm.
 * hipBLAS operates in column-major order; our tensors are row-major.
 * We handle this by swapping A/B and transposing: C^T = B^T @ A^T.
 *
 * This wrapper provides a row-major interface on top of column-major hipBLAS.
 *
 * Precision on gfx1201 (RDNA4): there is no TF32 datapath, so plain FP32 sgemm
 * runs on the vector ALUs with no matrix-core acceleration. The BF16-in /
 * FP32-accumulate GemmEx path below is the one that reaches the WMMA matrix
 * cores, and is the fast path on this GPU.
 */
 ...
 * as HIPBLAS_STATUS_EXECUTION_FAILED even though the gemm itself is fine. */
static void check_cublas_status(hipblasStatus_t status, const char* op, const char* file, int line) {
    if (status == HIPBLAS_STATUS_SUCCESS) return;
    static bool reported = false;
    if (!reported) {
        hipError_t lingering = hipGetLastError();
        fprintf(stderr,
            "cuBLAS %s failed: status=%d at %s:%d  (prior CUDA error: %s)\n",
            op, (int)status, file, line, hipGetErrorString(lingering));
        reported = true;
    }
    abort();
}
...
__global__ void bf16_to_f32_kernel(const __hip_bfloat16* __restrict__ in, float* __restrict__ out, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
...
void cublas_init() {
    if (g_cublas_handle == nullptr) {
        ...
        if (status != HIPBLAS_STATUS_SUCCESS) {
            fprintf(stderr, "hipBLAS init failed: %d\n", status);
        }
        /* The CUDA build set CUBLAS_TF32_TENSOR_OP_MATH here. RDNA4 has no TF32
         * datapath and the rocBLAS backend does not implement that math mode, so
         * there is nothing to enable -- FP32 gemms run at plain vector-FP32 rate.
         * Route throughput-sensitive matmuls through the BF16 wrappers instead. */
    }
}
...
void matmul_f32(
     ...
     * hipblasSgemm(N, N, n, m, k, &alpha, B, n, A, k, &beta, C, n)
     */
...
void matmul_f32_bt(
     ...
     * but B is row-major [n,k] = col-major B^T[k,n], so we need HIPBLAS_OP_T on B
     * Actually: hipblasSgemm(T, N, n, m, k, alpha, B, k, A, k, beta, C, n)
     */
    hipblasSgemm(g_cublas_handle,
...
void matmul_f32_batched(
    ...
    hipblasSgemmStridedBatched(g_cublas_handle,
        HIPBLAS_OP_N, HIPBLAS_OP_N,
        n, m, k,
        &alpha,
        B, n, strideB,
 ...
 */
void convert_f32_to_bf16(const float* in, __hip_bfloat16* out, int n) {
    int threads = 256;
    int blocks = (n + threads - 1) / threads;
    f32_to_bf16_kernel<<<blocks, threads>>>(in, out, n);
}

void convert_bf16_to_f32(const __hip_bfloat16* in, float* out, int n) {
    int threads = 256;
    int blocks = (n + threads - 1) / threads;
...
void matmul_bf16_from_f32(
    const float* A, const float* B, float* C,
    __hip_bfloat16* scratch_a, __hip_bfloat16* scratch_b,
    int m, int n, int k
) {
    int threads = 256;
    int a_n = m * k, b_n = k * n;
    ...
    CUBLAS_CHECK(hipblasGemmEx(g_cublas_handle,
        HIPBLAS_OP_N, HIPBLAS_OP_N,
        n, m, k,
        &alpha,
        scratch_b, HIP_R_16BF, n,
        scratch_a, HIP_R_16BF, k,
        &beta,
        C, HIP_R_32F, n,
        HIPBLAS_COMPUTE_32F,
...
void matmul_bf16_from_f32_batched(
    ...
    CUBLAS_CHECK(hipblasGemmStridedBatchedEx(g_cublas_handle,
        HIPBLAS_OP_N, HIPBLAS_OP_N,
        n, m, k,
        &alpha,
        scratch_b, HIP_R_16BF, n, strideB,
        scratch_a, HIP_R_16BF, k, strideA,
        &beta,
        C, HIP_R_32F, n, strideC,
        batch_count,
        HIPBLAS_COMPUTE_32F,
        HIPBLAS_GEMM_DEFAULT
    ));
...
void matmul_bf16_from_f32_bt_batched(
    ...
    CUBLAS_CHECK(hipblasGemmStridedBatchedEx(g_cublas_handle,
        ...
        n, m, k,
        &alpha,
        scratch_b, HIP_R_16BF, k, strideB,
        scratch_a, HIP_R_16BF, k, strideA,
        &beta,
        C, HIP_R_32F, n, strideC,
        batch_count,
...
void matmul_bf16_from_f32_at_batched(
    ...
    CUBLAS_CHECK(hipblasGemmStridedBatchedEx(g_cublas_handle,
        ...
        &alpha,
        scratch_b, HIP_R_16BF, n, strideB,
        scratch_a, HIP_R_16BF, m, strideA,
        &beta,
        C, HIP_R_32F, n, strideC,
        batch_count,
        HIPBLAS_COMPUTE_32F,
